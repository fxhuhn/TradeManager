...
@pytest.mark.asyncio
async def test_csv_directory_watcher_success_rename(tmp_path: Path, mock_config: Config) -> None:
    """
    ...
    with patch("app.services.importer.run_csv_import", new_callable=AsyncMock) as mock_import:
...
@pytest.mark.asyncio
async def test_csv_directory_watcher_error_rename(tmp_path: Path, mock_config: Config) -> None:
    """
