...
async def process_trade_group(
    ...
    entry_order = next(
        (order for order in orders if order.bracket_role == 'ENTRY'), 
        None
    )
    child_orders = [
        order for order in orders if order.bracket_role != 'ENTRY'
    ...
    if not entry_order:
        logger.error(
            "Keine ENTRY-Order in Gruppe vorhanden", 
            trade_group_id=trade_group_id
        )
        return
    ...
    if entry_order.status == "Created":
        logger.info(
            "Normaler Bracket: Verarbeite ENTRY-Order", 
            trade_group_id=trade_group_id
        )
    ...
    for iteration_index, child in enumerate(child_orders):
        if child.status == "Created":
            ...
            if not is_post_fill:
                ...
                logger.info(
                    "Post-Fill Child: Sende Child ohne parentId", 
                    child_id=tws_order_id
                )
...
async def execution_worker(
    ...
    while True:
        try:
            ...
            logger.error(
                "Fehler im Execution Worker Loop", 
                error=str(exception)
            )
